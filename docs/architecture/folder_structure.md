...
| `database/` | Everything database-related. | Schema definition, connection handling, the Storage Manager (upsert/query logic), versioned migrations. | Business logic about what makes a "good" opportunity (that's `intelligence/`). |
...
| `data/` | Data directories and any persisted local state. | Persisted state. | Code. |
